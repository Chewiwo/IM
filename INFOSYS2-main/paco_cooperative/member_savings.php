...
<main class="savings-page">
    ...
    <?php if ($accounts): ?>
        ...
        <section class="history">
            <div style="align-items:center;display:flex;flex-wrap:wrap;gap:12px;justify-content:space-between"><h2><?php echo htmlspecialchars($selected_account['account_type'], ENT_QUOTES, 'UTF-8'); ?> History</h2><span><a href="statement.php?account_id=<?php echo (int) $selected_account['account_id']; ?>&from=<?php echo urlencode($from_date); ?>&to=<?php echo urlencode($to_date); ?>&type=<?php echo urlencode($transaction_type); ?>" target="_blank" rel="noopener">Print / Save PDF</a> · <a href="member_savings.php?account_id=<?php echo (int) $selected_account['account_id']; ?>&from=<?php echo urlencode($from_date); ?>&to=<?php echo urlencode($to_date); ?>&type=<?php echo urlencode($transaction_type); ?>&download=csv">Download CSV</a></span></div>
            <form method="GET" style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:18px"><input type="hidden" name="account_id" value="<?php echo (int) $selected_account['account_id']; ?>"><label>From <input type="date" name="from" value="<?php echo htmlspecialchars($from_date, ENT_QUOTES, 'UTF-8'); ?>"></label><label>To <input type="date" name="to" value="<?php echo htmlspecialchars($to_date, ENT_QUOTES, 'UTF-8'); ?>"></label><label>Type <select name="type"><option value="">All</option><option value="deposit" <?php echo $transaction_type === 'deposit' ? 'selected' : ''; ?>>Deposits</option><option value="withdrawal" <?php echo $transaction_type === 'withdrawal' ? 'selected' : ''; ?>>Withdrawals</option><option value="transfer" <?php echo $transaction_type === 'transfer' ? 'selected' : ''; ?>>Transfers</option></select></label><button type="submit">Filter</button></form>
            <?php if ($transactions): ?>
