...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($username) && !empty($password)) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            record_audit($pdo, (int) $user['id'], 'login', 'Successful login', 'success');
            ...
            if ($user['role'] === 'admin') {
                ...
                session_destroy();
                $error = "This account has an invalid role. Please contact PASCCO support.";
            }
            exit;
        } else {
            record_audit($pdo, null, 'login', 'Failed login attempt for ' . $username, 'failed');
