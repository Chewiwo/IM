...
function pascco_member_header(string $title): void
{
    ...
    $member_links = [
        ...
        ['membership_apply.php', 'Become Member'],
    ...
    <header class="pascco-global-header">
        ...
        <!--
        <nav class="pascco-global-nav" aria-label="Member navigation">
            <?php foreach ($member_links as [$url, $label]): ?><a class="<?php echo $current_page === $url ? 'is-active' : ''; ?>" href="<?php echo $url; ?>" <?php echo $current_page === $url ? 'aria-current="page"' : ''; ?>><?php echo $label; ?></a><?php endforeach; ?>
        </nav>
        -->
        <button class="pascco-menu-toggle" type="button" aria-label="Toggle navigation" aria-expanded="false">
            <span></span><span></span><span></span>
        </button>
        <div class="pascco-global-account"><span class="pascco-account-name"><?php echo htmlspecialchars($_SESSION['username'] ?? 'Member', ENT_QUOTES, 'UTF-8'); ?></span><a href="logout.php">Logout</a></div>
    </header>
    <?php
}
...
function pascco_admin_header(): void
{
    ...
    <header class="pascco-global-header">
        ...
        <div class="pascco-global-account"><span>Administrator</span><a href="logout.php">Logout</a></div>
    </header>
    <?php
}
...
function pascco_public_header(string $action_label, string $action_url): void
{
    ...
    <header class="pascco-global-header">
        ...
        <div class="pascco-global-account"><a href="<?php echo htmlspecialchars($action_url, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($action_label, ENT_QUOTES, 'UTF-8'); ?></a></div>
    </header>
    <?php
}
...
function pascco_global_styles(): void
{
    ...
    <style>
        ...
        .pascco-global-header { align-items: center; background: linear-gradient(90deg, #071d49, #1456a0); color: white; display: flex; gap: 20px; min-height: 78px; padding: 12px clamp(18px, 5vw, 72px); position: relative; }
        .pascco-global-brand { align-items: center; color: white; display: flex; gap: 10px; margin-right: auto; text-decoration: none; }
        ...
        .pascco-global-nav { display: flex; flex-wrap: wrap; gap: 3px; justify-content: flex-end; }
        .pascco-global-nav a, .pascco-global-account a { border-bottom: 2px solid transparent; color: white; padding: 9px 10px; text-decoration: none; white-space: nowrap; }
        .pascco-global-nav a:hover, .pascco-global-nav a.is-active, .pascco-global-account a:hover { border-bottom-color: #e7b84b; color: #ffe7a1; }
        .pascco-global-account { align-items: center; display: flex; gap: 8px; margin-left: auto; white-space: nowrap; }
        .pascco-account-name { max-width: 150px; overflow: hidden; text-overflow: ellipsis; }
        .pascco-global-account a { border: 1px solid #e7b84b; color: #ffe7a1; }
        ...
        @media (max-width: 980px) { .pascco-global-header { align-items: flex-start; flex-wrap: wrap; } .pascco-global-nav { flex: 1 1 100%; justify-content: flex-start; order: 3; overflow-x: auto; padding-bottom: 2px; } .pascco-global-nav a { flex: 0 0 auto; } .pascco-global-account { margin-left: auto; } }
        ...
        @media (max-width: 420px) {
            ...
            .pascco-global-account a { padding: 8px 10px; }
        }
    </style>
    <script>
