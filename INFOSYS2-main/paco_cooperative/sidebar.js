(function () {
  function initPASCCOSidebar() {
    ...
    // 3. Inject CSS Styles with high priority
    ...
    // 4. Extract User's Name dynamically from header DOM
    ...
    headerNodes.forEach(header => {
      ...
      const nameNode = textNodes.find(el => el.textContent.includes("maria_santos") || el.textContent.includes("_") || el.classList.contains("user-name"));
      if (nameNode && nameNode.textContent.trim()) {
        userName = nameNode.textContent.trim().replace("Logout", "").trim();
      }
    });

    // 5. Create Overlay & Sidebar DOM elements
    ...
    sidebar.innerHTML = `
      ...
      <ul class="menu-links">
        ...
        <li><a href="member_savings.php"><i class='bx bx-piggy-bank'></i><span>My Savings</span></a></li>
        ...
        <li><a href="membership_apply.php"><i class='bx bx-id-card'></i><span>Become Member</span></a></li>
    ...
    // 6. Detect current URL and automatically set active menu link
    ...
    // 7. Create Hamburger Button inside existing page Header
    ...
    const headerEl = document.querySelector(".navbar, header, .header, .site-header, .header-wrap, nav");
    if (headerEl) {
